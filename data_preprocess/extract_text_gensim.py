...
output_files = ['features/text/train_text{}_100dim.ark'.format(pretrained_flag),
                'features/text/dev_text{}_100dim.ark'.format(pretrained_flag)]
    ...
    with open(outputfile, 'wb') as fd:
        for speaker in tqdm(speakers):
            ...
            transcript_file = os.path.join(
                args.transcriptdir, '{}_TRANSCRIPT.csv'.format(speaker))
            if not os.path.exists(transcript_file):
                log.warning("Speaker file does not exist {}".format(transcript_file))
                continue
